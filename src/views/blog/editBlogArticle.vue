...
export default {
  created: function() {
    ...
    this.queryBlogDic();
    //p为文章ID  带参数过来表示 修改
    var articleId = this.$route.query.p;
    if(articleId != undefined){
      this.queryBlogArticleById(articleId,this.myheaders);
      this.isSave = false;
    }
  },
  data() {
    ...
    submitForm(formName) {
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          //保存
          blogApi.updateBlogArticle(this.ruleForm,this.myheaders)
          .then(res => {
            if (res.code == 200) {
              console.log(res);
              this.$notify({
        ...
        blogApi.queryBlogArticleById(id,headers).then(res => {
            if (res.code == 200) {
              ...
              if(data.labels!=undefined){
                tepm = data.labels.map(item => {//把字符串变成数字
                  return item*1;
                });
                data.labels = tepm;
              }
              ...
              // }
              // //心情
              // if(TkApi.isNotEmpty(data.mood)){
              //   this.showIcon(data.mood,"face");
              // }
              this.ruleForm = data;
            }
        })
